...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val zoneDf = spark.sql("select rectangle, count(point) as pointsCount from joinResult group by rectangle order by rectangle")
    zoneDf.createOrReplaceTempView("zoneResult")
    return zoneDf.coalesce(1)
  }
